...
function getSheetWithRetry(name, attempts) {
  var maxAttempts = attempts || 3;
  var delaysMs = [300, 600];
  var lastErr = null;
  ...
  for (var i = 0; i < maxAttempts; i++) {
    try {
      ...
      lastErr = err;
      if (i < maxAttempts - 1) {
        Utilities.sleep(delaysMs[i] || delaysMs[delaysMs.length - 1]);
      }
    }
  }

  throw lastErr || new Error('Failed to open sheet: ' + name);
}
...
function getInventory() {
  const sheet = getSheetWithRetry(SHEETS.INVENTORY);
  const data = sheet.getDataRange().getValues();
  const items = [];
...
function saveSale(body) {
  ...
  const sheet = getSheetWithRetry(SHEETS.SALES);
  migrateSalesSheet();
